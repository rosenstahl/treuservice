...
// Erweitern der Typen für pvMontageData basierend auf der JSON-Struktur
type ExtendedPVMontageData = typeof pvMontageData & {
  testimonials?: {
    title: string;
    items: Array<{
      text: string;
...
export default function PVMontagePage() {
  ...
  const data = pvMontageData as ExtendedPVMontageData
      ...
      <Section className="bg-[#F8FAFC] py-16">
        <Container>
          <div className="text-center mb-12">
            ...
            <H2 className="text-[#222425]">Die Vorteile Ihrer PV-Anlage</H2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            {data.qualitaetsversprechen.items.map((item, index) => (
                ...
                <CardContent className="p-6">
                  <div className="flex flex-col h-full">
                    <div className="mb-4 text-[#009FD8]">
                      ...
                      {index === 1 && <Shield className="h-8 w-8" />}
                      {index === 2 && <Sparkles className="h-8 w-8" />}
                      {index === 3 && <Clock className="h-8 w-8" />}
                      {index === 4 && <Zap className="h-8 w-8" />}
                    </div>
                    <h3 className="text-xl font-semibold mb-2 text-[#222425]">{item.split(':')[0]}</h3>
                    <p className="text-sm text-gray-600">{item.split(':')[1] || item}</p>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="max-w-xl">
              ...
              <H2 className="mb-6 text-[#222425]">{data.warumTreuService.title}</H2>
              <Paragraph className="text-gray-600 text-lg">
                Wir von TREU Service sind Ihr verlässlicher Partner für die professionelle Installation von Photovoltaikanlagen. Mit jahrelanger Erfahrung sorgen wir für eine fachgerechte und effiziente Montage Ihrer Solaranlage.
              </Paragraph>
              <div className="mt-8 flex flex-wrap gap-3">
                {data.vorteile.finanzielle.slice(0, 3).map((item, index) => (
      ...
      <Section className="bg-[#F8FAFC] py-16">
        <Container>
          <div className="text-center mb-12">
            ...
            <H2 className="mb-4 text-[#222425]">{data.spezialisierteLoesungen.title}</H2>
            <Paragraph className="text-gray-600 max-w-2xl mx-auto">
              Wir bieten maßgeschneiderte Lösungen für verschiedene Dachtypen und Anforderungen.
            </Paragraph>
          </div>
            ...
            <Card className="shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border-0">
              <div className="p-1 bg-[#009FD8]">
                <div className="py-3 px-4 flex items-center gap-3">
                  ...
                  <h3 className="text-white font-medium">Dachmontage</h3>
                </div>
              </div>
              <CardContent className="p-6">
                <ul className="space-y-3">
                  {data.spezialisierteLoesungen.services.slice(0, 3).map((service, idx) => (
                    <li key={idx} className="flex items-start">
                      <ArrowRight className="w-4 h-4 text-[#009FD8] mr-2 mt-1 flex-shrink-0" />
                      <span className="text-gray-700">{service.title}: {service.description.split('.')[0]}</span>
                    </li>
                  ))}
                </ul>
              </CardContent>
            ...
            <Card className="shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border-0">
              <div className="p-1 bg-[#009FD8]">
                <div className="py-3 px-4 flex items-center gap-3">
                  ...
                  <h3 className="text-white font-medium">Spezialinstallationen</h3>
                </div>
              </div>
              <CardContent className="p-6">
                <ul className="space-y-3">
                  {data.spezialisierteLoesungen.services.slice(3, 5).map((service, idx) => (
                    <li key={idx} className="flex items-start">
                      <ArrowRight className="w-4 h-4 text-[#009FD8] mr-2 mt-1 flex-shrink-0" />
                      <span className="text-gray-700">{service.title}: {service.description.split('.')[0]}</span>
                    </li>
                  ...
                  <li className="flex items-start">
                    <ArrowRight className="w-4 h-4 text-[#009FD8] mr-2 mt-1 flex-shrink-0" />
                    <span className="text-gray-700">Batteriespeicher: Erhöhen Sie Ihren Eigenverbrauch und werden Sie unabhängiger vom Stromversorger</span>
                  </li>
                </ul>
              </CardContent>
            </Card>
        ...
        <Section className="bg-white py-16">
          <Container size="small">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {data.preise.examples.map((example, index) => (
                <Card key={index} className="shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden text-center border-0">
                  <CardContent className="p-6">
                    <div className="mb-3 p-3 rounded-full bg-[#F8FAFC] inline-block">
                      {index === 3 ? (
                        <Battery className="h-8 w-8 text-[#009FD8]" />
                      ) : (
                        <Sun className="h-8 w-8 text-[#009FD8]" />
                      )}
                    </div>
      ...
      <Section className="bg-[#F8FAFC] py-16">
        <Container>
          <div className="text-center mb-12">
            ...
            <H2 className="mb-4 text-[#222425]">{data.arbeitsweise.title}</H2>
          </div>
      ...
      <Section className="bg-[#F8FAFC] py-16">
        <Container size="small">
          <div className="text-center mb-12">
            ...
            <H2 className="mb-4 text-[#222425]">{data.faq.title}</H2>
          </div>
      ...
      <Section id="kontakt" className="bg-white py-16">
        <Container size="small">
          <div className="text-center mb-12">
            ...
            <H2 className="mb-4 text-[#222425]">{data.kontakt.formTitle}</H2>
            <Paragraph className="text-gray-600 max-w-2xl mx-auto">
              {data.kontakt.formSubtitle}
            </Paragraph>
          </div>
          <PVMontageWizard />
        </Container>
      ...
      <Section className="bg-[#009FD8] text-white py-10">
        <Container size="small">
          <div className="text-center">
            <h2 className="text-2xl font-bold mb-4">{data.kontakt.description}</h2>
