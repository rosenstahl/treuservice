...
export const PreisUebersichtStep: React.FC<PreisUebersichtStepProps> = ({
  ...
  const getZusatzleistungenKosten = () => {
    let kosten = 0
    if (formData.zusatzleistungen.reinigung) kosten += 150
    if (formData.zusatzleistungen.entsorgungsnachweis) kosten += 50
    return kosten
  }
...
Geschätzter Gesamtpreis: ${formatPrice(formData.preisschaetzung + getZusatzleistungenKosten())}
...
Diese Anfrage wurde über das Kontaktformular der Website gesendet.
  ...
  // Berechne Gesamtpreis (Basispreis + Zusatzleistungen)
  const gesamtPreis = formData.preisschaetzung + getZusatzleistungenKosten()
  ...
  return (
    ...
    >
      {isSubmitted ? (
        ...
        // Zusammenfassung und Preisübersicht
        <div className="space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-medium text-gray-900">Übersicht & Preisschätzung</h2>
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Ihre Preisschätzung</h3>
            
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-gray-600">Basispreis Entrümpelung:</span>
                <span className="font-medium">{formatPrice(formData.preisschaetzung)}</span>
              </div>
              ...
              {formData.zusatzleistungen.entsorgungsnachweis && (
                <div className="flex justify-between">
                  <span className="text-gray-600">Entsorgungsnachweis:</span>
                  <span className="font-medium">{formatPrice(50)}</span>
                </div>
              )}
              
              <div className="pt-3 border-t border-blue-200 flex justify-between">
                <span className="text-lg font-medium text-gray-800">Geschätzter Gesamtpreis:</span>
                <span className="text-lg font-bold">{formatPrice(gesamtPreis)}</span>
              </div>
            </div>
            
            <div className="mt-4">
              <p className="text-xs text-gray-500">
                * Diese Preisschätzung basiert auf Ihren Angaben und kann vom tatsächlichen Preis abweichen. 
                Die genaue Berechnung erfolgt nach einem persönlichen Besichtigungstermin.
              </p>
            </div>
          </div>
