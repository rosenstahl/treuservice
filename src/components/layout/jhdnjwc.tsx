...
// Importiere nur den Weather-Context
...
// Formatierungsfunktion für Temperatur
const formatTemperature = (temp: number | undefined) => temp !== undefined ? Math.round(temp) : '--';

// Standort-Picker Komponente - vereinfacht
const LocationPicker = () => {
  const params = useParams();
  const locale = (params.locale as string) || "de";
  
  // Verwende den Weather-Context direkt für aktuelle Daten
  const { 
    isLoading, 
    error, 
    location, 
...
// Einfaches Wetter-Widget für den Header - komplett neu geschrieben
const WeatherWidget = () => {
  const params = useParams();
  const locale = (params.locale as string) || "de";
  
  // Direkter Zugriff auf Weather-Context 
  const { weather, location } = useWeather();
  
  // Wenn keine Daten, zeige Ladezustand
  if (!weather) {
    return (
  ...
  // Status basierend auf Winterdienst-Flag
  const status = weather.winterServiceRequired ? 'required' : 'not-required';
  ...
  const getWeatherIcon = () => {
    ...
    } else if (icon === 'partly-cloudy-day' || icon === 'partly-cloudy-night' || icon === 'cloudy') {
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <Link href={`/${locale}/winterdienst`} className={`flex items-center gap-2 px-3 py-1 rounded-md border text-sm cursor-pointer
            ${statusColors[status]}`}>
            {getWeatherIcon()}
            <span className="flex items-center gap-1">
              <span>{formatTemperature(weather.current.temperature)}°C</span>
              <span className="w-2 h-2 rounded-full flex-shrink-0"
                style={{
                  backgroundColor: status === 'required' ? 'rgb(220, 38, 38)' : 'rgb(22, 163, 74)'
                }}
              />
            </span>
          </Link>
          ...
          <div className="space-y-2">
            ...
            <div className="text-xs pt-1 border-t border-gray-200 mt-1">
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                ...
                <span>{(weather.current.precipitation).toFixed(1)} mm</span>
              </div>
            </div>
          </div>
        </TooltipContent>
