...
import { Thermometer, MapPin, CloudSnow, ArrowDown, ArrowUp } from 'lucide-react';
...
const WeatherHeader: React.FC<WeatherHeaderProps> = ({ onRequestService }) => {
  const { isLoading, error, location, weather, lastUpdated, detectLocation } = useWeather();
        ...
        <div className="flex flex-col space-y-4">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-slate-500">
              {lastUpdated ? format(lastUpdated, 'HH:mm', { locale: de }) : '-'}
            </span>
          </div>
          ...
          {/* Winterdienst Button bei Bedarf */}
          {winterServiceNeeded && onRequestService && (
            <Button 
