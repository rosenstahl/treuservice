...
const WeatherHero: React.FC<WeatherHeroProps> = ({ scrollToContact }) => {
  ...
  const renderWeatherContent = () => {
    ...
    const winterServiceNeeded = weather.winterServiceStatus !== WinterServiceStatus.NOT_REQUIRED;

    return (
      <>
        {/* Warnungs-Banner */}
        {winterServiceNeeded && (
          <div className="backdrop-blur-sm rounded-xl p-4 border border-white/10 mb-6 bg-red-500/20">
            <div className="flex items-center text-white mb-2">
              <CloudSnow className="w-5 h-5 mr-2" />
              <span className="font-bold">Winterdienst wird empfohlen</span>
            </div>
            <p className="text-white/90 mb-4">
              Aktuelle Wetterbedingungen deuten auf {weather.iceRisk.risk === 'high' ? 'hohe' : 'mögliche'} Rutschgefahr hin. 
              {weather.iceRisk.description}
            </p>
            {scrollToContact && (
              <button
                onClick={scrollToContact}
                className="w-full bg-accent hover:bg-accent-hover text-white px-4 py-2 rounded-md transition-colors font-medium flex items-center justify-center"
              >
        ...
        <div className="backdrop-blur-sm rounded-xl p-6 mt-6 border border-white/10">
          ...
          {winterServiceNeeded && scrollToContact && (
            <button
              onClick={scrollToContact}
        ...
        <div className="mt-4 text-white/70 text-sm flex items-center justify-center">
          ...
          <span>
            Aktualisiert: {lastUpdated ? format(lastUpdated, 'HH:mm:ss', { locale: de }) : 'Unbekannt'}
          </span>
        </div>
      </>
    );
