...
export function WeatherProvider({ children, initialLocation }: WeatherProviderProps) {
  ...
  /**
   * Geocoding: Adresse zu Koordinaten umwandeln
   */
  const geocodeAddress = useCallback(async (address: string): Promise<{
    ...
    try {
      ...
      const geocodeUrl = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodedAddress}&key=${GOOGLE_GEOCODING_API_KEY}&region=de`;
  ...
  const reverseGeocode = useCallback(async (lat: number, lon: number): Promise<string> => {
    try {
      const reverseGeocodeUrl = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lon}&key=${GOOGLE_GEOCODING_API_KEY}&result_type=locality|sublocality|political`;
